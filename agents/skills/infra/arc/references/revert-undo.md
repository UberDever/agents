...
**WARNING:** `arc checkout` does NOT accept `--` as a separator between
revision and file paths. Unlike git, arc interprets `--` as a literal file
path, causing errors like `error: path '--' did not match any file(s) known
to arc`. Pass revision and paths as positional arguments. (Other commands
like `arc diff`, `arc log`, `arc reset`, `arc blame` accept `--` normally.)
...
| `arc reset --mixed HEAD~1` | Unstages | Keeps changes | Redo staging |
| `arc reset --hard HEAD~1` | Resets | Resets | Discard everything (DESTRUCTIVE) |

Always pass an explicit mode flag. Arc has no documented default mode, and
bare `arc reset` (no args) is a no-op, not a git-style mixed reset.
