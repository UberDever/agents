...
Note: in `arc show COMMIT:path` the path is root-relative and works from any directory; a CWD-relative one errors, or silently reads a same-named file from the root (see Monorepo Pitfalls in SKILL.md).
