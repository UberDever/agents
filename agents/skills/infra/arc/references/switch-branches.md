...
Server branches live under `users/<login>/<ticket-or-name>` (visible in `arc pr list` and Arcanum). Locally, name the branch without the prefix, using the format `TICKET-1234-short-description` (e.g., `TICKET-1234-fix-login-bug`); arc adds the namespace on push. Adding the prefix locally causes double-prefix — see `push-changes.md` for mechanics and fallback.
